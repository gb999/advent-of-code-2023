...
impl HandType {
    fn from_cards(cards: &String) -> HandType {
        ...
        let joker_count = chars.iter().filter(|&&c| c == 'J').count();

        if (1..5).contains(&joker_count) {
            // Remove jokers from occurences
            let joker_occurence = occurences.iter().enumerate()
                .find(|occ| *occ.1 == ((joker_count) as u32))
                .unwrap();

            occurences.remove(joker_occurence.0);

            // Add number of jokers to highest occuring cards
            let max_non_joker_opccurence = occurences.iter_mut()
                .max_by(|a,b| a.cmp(b))
                .unwrap();
            *max_non_joker_opccurence += joker_count as u32;
        } 
...
fn main() {
    let contents = fs::read_to_string("input.txt").expect("should be able to read file");
