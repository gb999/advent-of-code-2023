...
fn find_lowest_heatloss(matrix: &Vec<Vec<u32>>) -> u32 {
    ...
    // Need to add both directons to start, because of 4 block constraint
    heap.push(Reverse(Move{x:0,y:0,dirx:1,diry:0,n_dir:0,heatloss:0}));
    heap.push(Reverse(Move{x:0,y:0,dirx:0,diry:1,n_dir:0,heatloss:0}));
    ...
    while let Some(Reverse(Move{x, y, dirx, diry, n_dir, heatloss})) = heap.pop() {
        ...
        if x == matrix.len() -1 && y == matrix.len() - 1 && n_dir >= 4 {
            return heatloss as u32;
        }

        if n_dir < 10  { // continue going in the same direction 
        ...
        if n_dir < 4 { // can't turn if crucible hasn't moved at least 4 blocks
            continue;
        }
