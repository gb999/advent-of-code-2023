...
impl Mapping {
    ...
    fn map_interval(interval: Range<u64>, ) {
        // current interval : I0 (a, b) 
        // I1(c,d): find interval containing 'a' from intervals below
        // the common sections are I2 (a, min{b,d}:=m) , (m, ) 
        // while m != b {
            //m = min{b, d} 
            // map_interval(a , m)
            //a = m 
...
fn map_interval(interval:Range<u64>, mappings: Vec<Mapping>, level: usize) {
    if level == mappings.len() { return };
    // Split into subintervals, which map to the same interval
    let mut start = interval.start;
    let mut end;
    let current_mapping = mappings[level];

    
    while start != interval.end {
        let current_interval = current_mapping.src_ranges.iter().
        find(|&src_range| src_range.contains(&start));

        match current_interval {
            Some(_) =>{},
            None => {
                // order intervals by starting point
            },
        }
